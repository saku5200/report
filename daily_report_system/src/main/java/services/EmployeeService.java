...
public class EmployeeService extends ServiceBase {
     ...
     * @return バリデーションや登録処理中に発生したエラーのリスト
     */
    public List<FollowView>  follower(FollowView fv) {

        List<Follow> ff =  em.createNamedQuery(JpaConst.Q_REP_COUNT_ALL_MINE, Follow.class)
                .setParameter(JpaConst.JPQL_PARM_EMPLOYEE, FollowConverter.toModel(fv))
                .getResultList();

        return FollowConverter.toViewList(ff);
